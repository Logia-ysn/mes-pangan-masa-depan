...
export class T_getPurchaseOrders_headers {
    @IsNotEmpty({ message: 'authorization cannot be empty' })
    @IsString({ message: 'authorization must be a string' })
    authorization!: string
}
export class T_getPurchaseOrders_query {
    @IsOptional()
...
export type T_getPurchaseOrders = (request: {
    headers: T_getPurchaseOrders_headers
    query: T_getPurchaseOrders_query
}, response: Response) => Promise<ReturnType_0>;

export const method = 'get';
export const url_path = '/purchase-orders';
export const alias = 'T_getPurchaseOrders';
