...
export class T_getProductionLines_headers {
    @IsNotEmpty({ message: 'authorization cannot be empty' })
    @IsString({ message: 'authorization must be a string' })
    authorization!: string
}
export class T_getProductionLines_query {
    @IsOptional()
...
export type T_getProductionLines = (request: {
    headers: T_getProductionLines_headers
    query: T_getProductionLines_query
}, response: Response) => Promise<ReturnType_0>;

export const method = 'get';
export const url_path = '/production-lines';
export const alias = 'T_getProductionLines';
